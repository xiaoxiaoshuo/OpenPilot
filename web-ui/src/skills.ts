...
function drawSkills(loading = false): void {
  ...
  render(
    html`${listPageTpl({
      ...
      action: { label: t("skills.newSkill"), onClick: startCreate },
      search: {
        value: skillSearch,
      ...
      filters: html`<div class="skill-registry-controls">
          <div class="resource-tabs" role="group" aria-label="Filter by skill status">
            ${(
              [
                ["active", t("skills.active"), counts.active],
                ["archived", t("skills.archived"), counts.archived],
                ["all", t("skills.all"), counts.all],
              ] as const
            ).map(
              ([value, label, count]) =>
                html`<button
