...
export function getBaseModel(id: string, fallback?: { name: string; provider: string }): PiModel {
  ...
  if (fallback?.provider === "openrouter") {
    ...
    if (template) return cloneModel(template, id, fallback.name, fallback.provider);
  }
  if (fallback) {
    const template = builtinModel("gpt-5.5") ?? builtinModel("gpt-5.6-sol");
    if (template) return cloneModel(template, id, fallback.name, fallback.provider);
  }
  throw new Error(`Unsupported model: ${id}`);
}

function cloneModel(model: PiModel, id: string, name: string, provider = model.provider): PiModel {
  return { ...structuredClone(model), id, name, provider };
}
