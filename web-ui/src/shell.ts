...
import { errMessage, swallow } from "../../core/chassis/src/errors";
import { brandMark, brandName, icon, initials } from "./ui";
import { applyLangAttr, lang, setLang, t } from "./i18n";
...
const PORTAL_ATTEMPT_KEY = "openpilot.portal.signin.attempt";
...
export function renderSidebarTop(): void {
  ...
  render(
    html`
      <button
        class="new-chat"
        title=${splitState.active ? t("sessions.newSession") : t("sessions.newChat")}
        @click=${() => {
          closeSidebarOnNarrowView();
          if (!addBlankPane()) mainConversation().newChat();
        }}
      >
        ${icon(ICON.newChat, 17)}<span>${splitState.active ? t("sessions.newSession") : t("sessions.newChat")}</span>
      </button>
