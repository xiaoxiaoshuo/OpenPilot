...
export function recentProjectSeeds(contexts: readonly ProjectAwareContext[]): RecentProjectSeed[] {
  return contexts.map((context): RecentProjectSeed => {
    ...
    if (context.kind === "personal") return { scopeId: context.scopeId, name: t("contexts.personal"), kind: "personal" };
...
export function recencyGroup(ms: number, now = Date.now()): string {
  ...
  if (ms >= dayStart(0)) return t("sessions.today");
  if (ms >= dayStart(1)) return t("sessions.yesterday");
  if (ms >= dayStart(6)) return t("sessions.prev7");
  if (ms >= dayStart(29)) return t("sessions.prev30");
