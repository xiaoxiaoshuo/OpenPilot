...
export async function renderContexts(): Promise<void> {
  ...
  try {
    ...
    if (seq !== appState.viewRenderSeq || appState.currentView !== "contexts") return;
    contextsNotice = errMessage(e, t("contexts.loadFailed"));
  }
...
function contextMeta(c: CoreContext): { title: string; sub: string; glyph: IconNode } {
  if (c.project) {
    ...
    return {
      ...
      sub: t("contexts.memberCount", { n: memberCount }),
      glyph: Folder,
    };
  }
  if (c.kind === "personal") {
    return { title: t("contexts.personal"), sub: t("contexts.personalSub"), glyph: User };
  }
  if (c.kind === "group") {
    return {
      title: sharedContextLabel(c.scopeId, c.name) ?? t("contexts.groupDm"),
      sub: t("contexts.groupSub"),
      glyph: Users,
    };
  }
  return {
    title: sharedContextLabel(c.scopeId, c.name) ?? t("contexts.channel"),
    sub: t("contexts.channelSub"),
    glyph: Hash,
  };
}
...
function metaForScope(scopeId: string | null, fallbackName?: string | null): { title: string; glyph: IconNode } {
  ...
  if (scopeId?.startsWith("personal:") && scopeId !== personalScopeId())
    return { title: t("contexts.sharedSpace"), glyph: User };
  return { title: fallbackName?.trim() || t("contexts.personal"), glyph: User };
}
...
export function scopeFilterControl(current: string | null, onSelect: (scopeId: string | null) => void): TemplateResult {
  const label = current ? metaForScope(current).title : t("contexts.allContexts");
  ...
  return html`
    <div class="menu-control form-menu-control scope-filter">
      <button class="menu-button" type="button" aria-haspopup="menu" aria-expanded="false" @click=${toggleFormMenu}>
        ${icon(ListFilter, 14)}<span class="menu-label">${t("contexts.filterByLabel")}${label}</span>${icon(ChevronDown, 14)}
      </button>
      <div class="menu-popover" role="menu" hidden>
        <div class="menu-title">${t("contexts.filterBy")}</div>
        ${option(null, t("contexts.allContexts"), Boxes)}
...
function gridTpl(): TemplateResult {
  const status = contextsNotice || (contextsLoading && contextsState.list.length === 0 ? t("contexts.loadingProjects") : "");
  ...
  if (projects.length) projectList = html`<div class="grid project-grid">${projects.map(contextCard)}</div>`;
    ...
    projectList = html`<div class="empty compact project-empty">
      ${projectsFiltered ? t("contexts.noProjectsMatch") : t("contexts.noProjectsYet")}
    </div>`;
  }
  return html`
    <div class="project-grid-content">
      <div class="pane-head">
        <h1 class="pane-title">${t("contexts.title")}</h1>
        <div class="project-head-actions">
          <button
            class="pane-refresh"
            type="button"
            aria-label=${t("contexts.refreshProjects")}
            title=${t("contexts.refreshProjects")}
          ...
          <button
            ...
            type="button"
            aria-label=${t("contexts.newProject")}
            title=${t("contexts.newProject")}
            @click=${openCreateProject}
          >
            ${icon(FolderPlus, 15)}<span>${t("contexts.newProject")}</span>
          </button>
        </div>
      </div>
      <div class="list-toolbar project-toolbar">
        <label class="list-search"
          ><span class="sr-only">${t("contexts.searchProjects")}</span
          ><input
            data-focus-key="contexts-search"
            type="search"
            aria-label=${t("contexts.searchProjects")}
            placeholder=${t("contexts.searchProjects")}
            .value=${contextsQuery}
        ...
        <label class="list-select"
          ><span>${t("contexts.show")}</span>${fieldSelect({
            compact: true,
            ...
            options: [html`<option value="active">${t("contexts.activeOnly")}</option>`, html`<option value="all">${t("contexts.everything")}</option>`],
          })}</label
        >
      </div>
...
function contextCard(c: CoreContext): TemplateResult {
  ...
  const count = t("contexts.conversationCount", { n: c.sessionCount });
  ...
  return html`
    <button class="card context-card" type="button" @click=${() => selectContext(c.scopeId)}>
      ...
      <div class="card-meta">${count}${c.lastActivityAt ? ` ${t("contexts.activeSince", { time: relTime(c.lastActivityAt) })}` : ""}</div>
    </button>
  `;
}
  ...
  return html`
    <div class="context-detail">
      <button class="context-back" type="button" @click=${() => selectContext(null)}>
        ${icon(ArrowLeft, 15)}<span>${t("contexts.title")}</span>
      </button>
      <div class="context-detail-head">
        ...
        <div class="context-detail-titles">
          <h1 class="pane-title">
            ${title}
            ${c.isPrivate ? html`<span class="context-lock" title=${t("contexts.privateChannel")}>${icon(Lock, 14)}</span>` : nothing}
          </h1>
          <div class="context-sub">
            ${c.project ? sub : `${sub} ${t("contexts.personalCtxSub")}`}
          </div>
        </div>
          ...
          ${
            c.project
              ? html`<button class="btn context-add-member" type="button" @click=${() => toggleMemberPicker(c)}>
                  ${icon(UserPlus, 15)}<span>${t("contexts.addPeople")}</span>
                </button>`
              : nothing
          }
          <button class="btn primary context-new-chat" type="button" @click=${() => startChatIn(c)}>
            ${icon(Plus, 15)}<span>${t("sessions.newChat")}</span>
          </button>
        </div>
      </div>
            ...
            completelyEmpty
              ? html`
                  <section class="context-panel context-project-empty">
                    ...
                    <h2>${t("contexts.projectReady")}</h2>
                    <p>${t("contexts.projectReadyBody")}</p>
                  </section>
                `
              : html`
                  <section class="context-panel context-conversations" aria-labelledby="context-conversations-title">
                    <div class="context-panel-heading">
                      <h2 class="context-panel-title" id="context-conversations-title">${t("contexts.conversations")}</h2>
                    ...
                    ${
                      sessions.length
                        ...
                        : html`<div class="context-inline-empty">${t("contexts.noConversations")}</div>`
                    }
                  </section>
...
function memberLabel(context: CoreContext, principalId: string): string {
  if (principalId === appState.me?.user) return t("contexts.you");
...
function projectMembersSection(context: CoreContext): TemplateResult {
  ...
  return html`
    <section class="context-panel project-members" aria-labelledby="project-people-title">
      <div class="context-panel-heading">
        <h2 class="context-panel-title" id="project-people-title">${t("contexts.people")}</h2>
        ...
        ${projectPeople(context).map((principalId) => {
          ...
          return html`
            <div class="project-member-row">
              ...
              ${principalId === project.ownerId ? html`<span class="badge">${t("contexts.owner")}</span>` : nothing}
              ${
                isProjectOwner(context) && principalId !== project.ownerId
                  ? html`<button
                      class="project-icon-button danger"
                      type="button"
                      aria-label=${t("contexts.removeMember", { name: label })}
                      title=${t("contexts.removeMember", { name: label })}
...
function memberPicker(context: CoreContext): TemplateResult {
  ...
  if (idle && contextsState.memberSearchedQuery && matches.length === 0) {
    emptyNote = contextsState.memberMatches.length
      ? t("contexts.everyoneAlreadyIn")
      : t("contexts.noMatchesFor", { q: contextsState.memberSearchedQuery });
  }
  let memberStatus = emptyNote;
  if (contextsState.memberSearching) memberStatus = t("contexts.searching");
  else if (contextsState.memberBusy) memberStatus = t("contexts.working");
  return html`
    <form class="project-member-picker" @submit=${(event: SubmitEvent) => void searchProjectMembers(event, context)}>
      <label for="project-member-search">${t("contexts.addPeople")}</label>
      <div class="project-member-search-row">
        ...
        <input
          ...
          autocomplete="off"
          maxlength="80"
          placeholder=${t("contexts.searchByName")}
...
function resourceSections(scopeId: string): TemplateResult | typeof nothing {
  ...
  if (!r) {
    return contextsState.resourcesLoading
      ? html`<div class="empty compact">${t("contexts.loadingResources")}</div>`
      : html``;
  }
  ...
  return html`
    ${r.files.length ? resourceGroup(t("contexts.files"), r.files.map(fileRow)) : nothing}
    ${
      r.skills.length
        ? resourceGroup(
            t("skills.title"),
    ...
    ${r.deployments.length ? resourceGroup(t("deploys.title"), r.deployments.map(deploymentRow)) : nothing}
  `;
}
...
async function manageCron(id: string, action: "enable" | "disable" | "delete"): Promise<void> {
  ...
  if (action === "delete" && !confirm(t("contexts.deleteCronConfirm"))) return;
  ...
  try {
    ...
    contextsState.resourcesNotice = errMessage(e, t("contexts.cronUpdateFailed"));
  } finally {
...
async function deleteScopeSkill(id: string): Promise<void> {
  const key = `skill:${id}`;
  if (resourceBusy.has(key)) return;
  if (!confirm(t("contexts.deleteSkillConfirm"))) return;
  resourceBusy.add(key);
  drawContexts();
  try {
    ...
    contextsState.resourcesNotice = errMessage(e, t("contexts.skillDeleteFailed"));
  } finally {
    resourceBusy.delete(key);
    drawContexts();
  }
  ...
  return html`
    <section class="context-panel context-resource-group">
      <div class="context-panel-heading context-resource-heading">
        ...
        <a href=${href}>${t("contexts.viewAll")}</a>
      </div>
        ...
        ${
          manage && !c.archived
            ? html`
                ...
                >
                  ${c.enabled ? t("contexts.disable") : t("contexts.enable")}
                </button>
                <button
    ...
    >
      <form @submit=${(event: SubmitEvent) => void createProject(event)}>
        <div class="project-dialog-head">
          ...
          <div><h2 id="project-dialog-title">${t("contexts.newProject")}</h2></div>
          <button
            class="project-icon-button"
            type="button"
            aria-label=${t("contexts.closeNewProject")}
            title=${t("common.close")}
        ...
        <div class="project-dialog-actions">
          <button class="btn" type="button" @click=${closeCreateProject}>
            ${contextsState.createSaving ? t("common.close") : t("common.cancel")}
          </button>
          <button class="btn primary" type="submit" ?disabled=${contextsState.createSaving}>
            ${icon(FolderPlus, 15)}<span>${contextsState.createSaving ? t("contexts.creating") : t("contexts.createProject")}</span>
          </button>
        </div>
      </form>
    </dialog>
...
async function createProject(event: SubmitEvent): Promise<void> {
  ...
  if (!name) {
    contextsState.createError = t("contexts.enterProjectName");
    drawContexts();
    queueMicrotask(() => document.querySelector<HTMLInputElement>("#project-name")?.focus());
    return;
  }
  ...
  try {
    ...
    contextsState.createSaving = false;
    contextsState.createError = errMessage(error, t("contexts.createFailed"));
    drawContexts();
    queueMicrotask(() => document.querySelector<HTMLInputElement>("#project-name")?.focus());
  }
}
...
async function searchProjectMembers(event: SubmitEvent, context: CoreContext): Promise<void> {
  ...
  if (query.length < 2) {
    contextsState.memberMatches = [];
    contextsState.memberSearchedQuery = "";
    contextsState.memberError = t("contexts.enterAtLeastTwo");
    drawContexts();
    return;
  }
...
async function runMemberSearch(context: CoreContext, query: string): Promise<void> {
  ...
  try {
    ...
    if (searchSeq !== memberSearchSeq || contextsState.memberProjectId !== projectId) return;
    contextsState.memberSearchedQuery = "";
    contextsState.memberError = errMessage(error, t("contexts.searchFailed"));
  } finally {
...
async function addProjectMember(context: CoreContext, member: DirectoryMatch): Promise<void> {
  ...
  try {
    ...
    if (resetSeq !== contextsResetSeq) return;
    contextsState.memberError = errMessage(error, t("contexts.addFailed"));
  } finally {
...
async function removeProjectMember(context: CoreContext, principalId: string): Promise<void> {
  ...
  try {
    ...
    if (resetSeq !== contextsResetSeq) return;
    contextsState.memberError = errMessage(error, t("contexts.removeFailed"));
  } finally {
    if (resetSeq === contextsResetSeq) {
      contextsState.memberBusy = false;
      drawContexts();
...
async function loadScopeResources(scopeId: string): Promise<void> {
  ...
  try {
    ...
    if (stale()) return;
    contextsState.resourcesNotice = errMessage(e, t("contexts.resourcesLoadFailed"));
  } finally {
    if (!stale()) {
      contextsState.resourcesLoading = false;
      drawContexts();
